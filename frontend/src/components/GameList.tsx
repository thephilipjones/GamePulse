 ...
 *
 * Handles five states:
 * 1. Loading: Shows skeleton placeholders while fetching initial data
 ...
 * 5. Success: Renders game cards with team names, scores, and colors
 *
 * Uses TanStack Query via useGames hook for efficient data fetching
 * with automatic caching, retry logic, and 15-minute polling.
 *
 * @example
 * ```tsx
 * <Container>
...
export function GameList() {
  ...
  // Error state: Show warning with cached data if available, otherwise show error
  if (isError && !data) {
    return (
  ...
  if (isError && data) {
    // Show warning banner with cached data below
    return (
      <Stack gap={4}>
